...
```
Pi (always on) ─► CEC at boot: TV on + select this input
                     │
...
On the Pi the live config is **`/data/config.toml`** (on the `/data` partition).
The repo's [`config.toml`](config.toml) is just the template copied there on
install. The slideshow re-reads it every 30 s, so most changes apply without a
restart:
...
| Kiosk crash-loops with `The profile appears to be in use by another computer (<other-hostname>)` in `kiosk.log` | A reused SD card kept its old `frame-data` partition from a *different* device — `sudo rm -f /data/chromium-profile/Singleton{Lock,Cookie,Socket} && sudo pkill -x sway`. See `docs/tv-and-hardware.md` re: fully erasing a card before reusing it for another device |
